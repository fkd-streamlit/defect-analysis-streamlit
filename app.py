...
from skimage import measure, morphology, filters, segmentation, feature, exposure, util
...
def binarize(img: np.ndarray,
    ...
    if method == "otsu":
        ...
        bin_local = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
...
def split_touching_particles(bin_img: np.ndarray,
    ...
    """
    距離変換＋Watershedで接触粒子を分離（ラベル画像を返す）。
    min_distance_px: 局所極大の最小距離
    h_max: h-maxima の隆起（大きいほど保守的）
    """
...
def min_area_rect_feret(coords_rc: np.ndarray) -> Tuple[float, float, float]:
    """
    cv2.minAreaRect による外接回転矩形 (最小面積) から Feret の最大/最小を推定。
    Returns: (feret_max_px, feret_min_px, angle_deg)
    """
    # coords_rc: (row, col) -> (x, y)
    pts = np.fliplr(coords_rc).astype(np.float32)  # to (x,y)
    if len(pts) < 5:
        # 点が少なすぎる場合のフォールバック：bbox
...
def overlay_labels(img_gray: np.ndarray,
    ...
    """粒子の輪郭をアスペクト比で色分けし、IDを描画。"""
    img_color = cv2.cvtColor(img_gray, cv2.COLOR_GRAY2BGR)
    contours, _ = cv2.findContours((label_img > 0).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    # 色マップ（緑: 等軸、黄: 中、赤: 細長）
...
def plot_distributions(df: pd.DataFrame, xcols: List[str], group: Optional[str] = None):
    """ヒスト／箱ひげ／CDF を簡易描画（matplotlib→st.pyplot）。"""
    if df.empty:
        st.info("有効な粒子がありません。しきい値・面積フィルタを調整してください。")
        return
    for x in xcols:
        st.subheader(f"ヒストグラム：{x}")
...
def process_one_image(name: str,
                      ...
                      show_id: bool) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
    """1画像を解析して結果DFと画像（原・二値・ラベル）を返却。"""
    ...
    overlay = overlay_labels(img_gray, label_img, df, aspect_bins, show_id) if not df.empty else cv2.cvtColor(img_gray, cv2.COLOR_GRAY2BGR)
...
with st.sidebar:
    ...
    um_per_px = compute_um_per_px(um_per_px_input,
        ...
        if f.name.lower().endswith(".zip"):
            with zipfile.ZipFile(io.BytesIO(f.read())) as zf:
                for info in zf.infolist():
                    if info.is_dir():
                        continue
                    if os.path.splitext(info.filename.lower())[-1] in [".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp"]:
    ...
    if len(results) == 0:
        ...
        for name, (img_gray, bin_img) in previews.items():
            ...
            with col[2]:
                st.image(cv2.cvtColor(overlays[name], cv2.COLOR_BGR2RGB),
                         caption="オーバーレイ（ID + アスペクト比色）", use_column_width=True, clamp=True)

        # ダウンロード（CSV / 注釈画像）
        st.markdown("### エクスポート")
        csv_bytes = df_all.to_csv(index=False).encode("utf-8-sig")
        st.download_button("📥 粒子特性CSVをダウンロード", data=csv_bytes, file_name="grain_metrics.csv", mime="text/csv")

        # 個別の注釈画像を zip にまとめて提供
        with tempfile.TemporaryDirectory() as tmpd:
            ...
            with open(zip_path, "rb") as fz:
                st.download_button("🖼️ 注釈画像（ZIP）をダウンロード", data=fz.read(),
