...
# 1) 検出対象を切替可能：
#    - 白領域（材料/粒子）…従来通り
#    - 黒領域（材料内部の空隙/欠陥）…二値画像の黒を検出（背景は除外）
# 2) 欠陥マスク用のOpen/Closeを別に用意（サイドバー調整）
# 3) オーバーレイで輪郭を赤色描画（太さ調整、輪郭のみ表示も可）
# 4) Streamlit use_column_width 警告を解消（use_container_widthへ統一）
# 5) Watershedのmin_distanceをピーク抽出に反映（調整が効くように改善）
...
def extract_internal_black_defects(bin_clean_u8: np.ndarray,
    ...
    if assume_material_is_largest:
        ...
        # 白領域すべてを材料として扱う（画像によっては背景誤検出の可能性あり）
...
def split_touching_particles(bin_u8: np.ndarray,
    ...
    if h_max > 0:
        # h_maximaはboolだが、ここではピーク抑制の目安として利用
...
st.caption("ローカル/VSCode 実行用。単一画像・複数画像・ZIP一括に対応。検出対象（白/黒）を切替可能。")
...
if uploaded_files:
    ...
    if len(previews) == 0:
        ...
        if df_all.empty:
            ...
            st.success(f"解析完了：{len(results)} ファイル、{len(df_all)} 粒子/欠陥")
